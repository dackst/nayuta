...
def insert(filename, insert_col, compression_offset, length_limits = None):
    ...
    inputdata = []
    with open('{}.tsv'.format(filename), 'r', encoding = 'utf-8') as f:
        for line in f:
            line = line.rstrip('\r\n').split('\t')
...
##    unc_output = True
    if compression_offset == -1 or unc_output:
        with open(r'{}\{}.tbl'.format(
            OUTPUT_FOLDER, filename), 'wb') as f:
            f.write(filedata)
    else:
        with open(r'{}\{}.tbb'.format(
            OUTPUT_FOLDER, filename), 'wb') as f:
            with open(r'orig\{}.tbb'.format(filename), 'rb') as g:
...
for fldlist in ('fldlist0', 'fldlist1', 'fldlist2', 'fldlist3'):
    insert(fldlist, insert_col = 3, compression_offset = 0x1F,
           length_limits = {'name': 23, 'desc': 63, 'mission': 47})
insert('item',      insert_col = 3, compression_offset = -0x1,
       length_limits = {'name': 23, 'desc': 77})
insert('monslib',   insert_col = 3, compression_offset = 0x1F,
       length_limits = {'mon name': 31, 'line': 31})
insert('questlib',  insert_col = 3, compression_offset = 0x1F,
       length_limits = {'quest name': 47, 'client': 31, 'line': 51})
insert('foodarea',  insert_col = 2, compression_offset = 0x1F,
       length_limits = 25)
insert('helplib',   insert_col = 2, compression_offset = 0x1F,
       length_limits = 31)
insert('pc',        insert_col = 2, compression_offset = 0x23,
       length_limits = 15)
#has description:
